        ...
        settings = [
          {
            ...
            modules-left = [
              "custom/icon"
              (lib.mkIf myconfig.programs.hyprland.enable "hyprland/workspaces")
              (lib.mkIf myconfig.programs.hyprland.enable "hyprland/window")
              (lib.mkIf myconfig.programs.niri.enable "niri/workspaces")
              (lib.mkIf myconfig.programs.niri.enable "niri/window")
            ];
            modules-center = [
              "mpris"
            ];
            modules-right = [
              ...
              "temperature"
              "memory"
              (lib.mkIf (!host.powersaveFeatured) "network")
              (lib.mkIf host.powersaveFeatured "battery")
              "pulseaudio"
              "clock"
            ];
            "custom/icon" = {
            ...
            "niri/window" = {
              "format" = "<span color='#222222' bgcolor='#d3869b' > WIN </span> {title}";
              "separate-outputs" = true;
            };
            "tray" = {
              "icon-size" = 15;
            ...
            "temperature" = {
              ...
              # "thermal-zone" = 2;
              "hwmon-path" =
                if host.name == "nixos-fishy" then
                  "/sys/class/hwmon/hwmon2/temp3_input"
                else if host.name == "noksaek" then
            ...
            "battery" = {
              "format" = "<span color='#222222' bgcolor='#b8bb26'> BAT </span> {capacity}%";
              "tooltip-format" = "Time to: {timeTo} - Draw: {power} - Health: {health}";
            };
          }
        ];
      };
